...
public class ControladorGestorInventario {
    ...
    public ControladorGestorInventario() {
        this.dao = new DAOAlmaceneroImpl();
    }

    public DefaultTableModel listarProductosInventario(JTable table) {
        ...
        model.addColumn("Cantidad en inventario");

        try {
            //DAOUsers dao = new DAOUsersImpl();
            dao.obtenerListaInventarios("").forEach((u) -> {
                Producto producto = null;
                try {
                    producto = dao.obtenerProductoPorId(u.getIdProductoInventariado());
                } catch (Exception ex) {
                    System.out.println(ex.getMessage());
                }
                
                model.addRow(new Object[]{u.getIdInventario(), u.getIdProductoInventariado(), u.getNombreProducto(),
                    producto.getCategoriaProducto().getNombreCategoria(), u.getCantidadProducto()});
            });
            return model;
        } catch (Exception e) {
    ...
    */
    ...
    public DefaultTableModel buscarProductosInventario(JTable table, String name) {
        ...
        try {
            dao.obtenerListaInventarios(name).forEach((u) -> {
                Producto producto = null;
                try {
                    producto = dao.obtenerProductoPorId(u.getIdProductoInventariado());
                } catch (Exception ex) {
